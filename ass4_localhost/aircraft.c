...
void connect_to_airport(int index, char **argv) {
    ...
    int error = getaddrinfo(LOCALHOST, argv[index], &hints, &ai);
    if (error != 0) {
        plane->connectFailed = true;
        freeaddrinfo(ai);
        return;
    }
    int planeSock = socket(AF_INET, SOCK_STREAM, 0);
    if (planeSock < 0) {
        plane->connectFailed = true;
        return;
    }
    int link = connect(planeSock, (struct sockaddr *)ai->ai_addr,
...
PlaneStatus connect_to_mapper(char **argv) {
    ...
    hints.ai_family = AF_INET;
    hints.ai_socktype = SOCK_STREAM;
    int error = getaddrinfo(LOCALHOST, plane->mapPort, &hints, &ai);
    if (error != 0) {
        freeaddrinfo(ai);
        return BAD_MAP_CONN;
    }
